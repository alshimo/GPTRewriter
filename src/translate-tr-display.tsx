...
interface Preferences {
  ...
  translateTrDisplayModel: string;
}

export default function TranslateTrDisplayCommand() {
  ...
  const translateText = async () => {
    try {
      ...
      const response = await processText({
        ...
        commandName: "translateTrDisplay",
    ...
    return (
      <List>
        <List.Section title="Processing">
          <List.Item
            title="Translating..."
            subtitle="Please wait while we translate to Turkish"
            icon={Icon.Clock}
          />
        </List.Section>
      </List>

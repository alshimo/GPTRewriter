...
interface Preferences {
  ...
  translateFaDisplayModel: string;
}

export default function TranslateFaDisplayCommand() {
  ...
  const translateText = async () => {
    try {
      ...
      const response = await processText({
        ...
        commandName: "translateFaDisplay",
        preferences: preferences,
      });

      if (response) {
        setTranslation(response);
        showToast(Toast.Style.Success, "Translation to Persian completed");
      } else {
    ...
    return (
      <List>
        <List.Section title="Processing">
          <List.Item
            title="Translating..."
            subtitle="Please wait while we translate to Persian"
            icon={Icon.Clock}
          />
        </List.Section>
      </List>
